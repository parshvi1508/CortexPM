...
# Custom CSS to match login page style
def load_css():
    st.markdown("""
        <style>
...
def show_dashboard_content(role, selected_page):
    if selected_page == "Dashboard":
        ...
        with col1:
            ...
            with st.container(height=400, border=True):
                st.write("Activity 1 - Project submission")
                st.write("Activity 2 - Grade updated")
                st.write("Activity 3 - New assignment")
                st.write("Activity 4 - Feedback received")
...
def render_dashboard(role):
    st.set_page_config(page_title=f"CortexPM - {role}", layout="wide")
    init_session_state()
    load_css()
    
    # Create sidebar and get selected page
    selected_page = create_sidebar(role)
    
    # Main content
    show_dashboard_content(role, selected_page)
...
if __name__ == "__main__":
    # Example usage:
    # Change role to "Admin", "Faculty", or "Student" as needed
